...
class Frontier(object):
    '''
    Frontier class implements a search frontier using a
...
class ProgressionPlanning(object):
    ...
    def successor(self, state, action):
        ...
        return state.union(pos).difference(neg)

    def goal_test(self, state):
        ''' Return true if `state` is a goal state. '''
        goal = self.problem.goal
        return state.intersect(goal) == goal
    ...
    def solve(self, W, heuristics):
        ...
        num_explored = 0
        num_generated = 0
        explored = {initial_state: initial_node}
        ...
        while not frontier.is_empty():
            ...
            applicable_actions = self.applicable(state)

            for action in applicable_actions:
                successor_state = self.successor(state, action)
                num_generated += 1
                if (successor_state not in explored) or (node.g+1 < explored[successor_state].g):
                    successor_node = Node(
